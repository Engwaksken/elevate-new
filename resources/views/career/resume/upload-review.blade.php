@extends('layouts.app')
@section('title','Review Resume Upload - ElevateHer360')
@section('content')
<div class="career-page">
<div class="page-header">
    <div><span class="eyebrow">Resume Import</span><h1>Review Uploaded Resume</h1><p>Your current resume will not be overwritten automatically.</p></div>
            ...
            <section class="eh-tab-pane active" data-eh-pane="status">
                <div class="career-form-panel">
                    ...
                    @if(in_array($upload->status,['uploaded','processing']))
                        <div class="processing-status" data-upload-status-url="{{ route('career.resume.upload.status',$upload) }}">
                            <span class="processing-spinner"></span>
                            <div><strong>Analysing your resume</strong><div>We are extracting your professional information. This page will refresh automatically when analysis finishes.</div></div>
                        </div>
                    @elseif($upload->status==='ready')
                        <div class="success-box"><i class="fas fa-circle-check"></i> Resume analysis completed. Review the extracted information before importing.</div>
                    @elseif($upload->status==='failed')
                        <div class="error-box">{{ $upload->parsing_error ?: 'Resume analysis failed.' }}</div>
                    @endif
            ...
            <section class="eh-tab-pane" data-eh-pane="data">
                <div class="career-form-panel">
                    @if($upload->status==='ready')
                        ...
                        <div class="ai-note">Review the extracted information carefully. Importing creates a new structured resume; it does not overwrite an existing resume.</div>
                        <form method="POST" action="{{ route('career.resume.upload.import',$upload) }}">@csrf
                            <button class="btn btn-primary"><i class="fas fa-file-import"></i> Import Into Resume Builder</button>
                        </form>
                    @else
