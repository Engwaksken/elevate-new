...
@php
    $participantShell = auth()->check()
        && method_exists(auth()->user(), 'isStaff')
        && ! auth()->user()->isStaff();
@endphp
        ...
        <div class="participant-app-main">
            <header class="participant-mobile-header">
                ...
                @if(Route::has('notifications.index'))
                    <a href="{{ route('notifications.index') }}" class="participant-mobile-action" aria-label="Notifications"><i class="fas fa-bell"></i></a>
                @else
                    <span></span>
                @endif
            </header>
            ...
            <main id="main-content" class="site-main participant-site-main">
                @if(session('success'))<div class="success-box"><i class="fas fa-circle-check"></i><span>{{ session('success') }}</span></div>@endif
                @if(session('error'))<div class="error-box"><i class="fas fa-circle-exclamation"></i><span>{{ session('error') }}</span></div>@endif
                @if(session('warning'))<div class="warning-box"><i class="fas fa-triangle-exclamation"></i><span>{{ session('warning') }}</span></div>@endif
                @if(session('info'))<div class="info-box"><i class="fas fa-circle-info"></i><span>{{ session('info') }}</span></div>@endif
                @if($errors->any())<div class="error-box"><strong><i class="fas fa-circle-exclamation"></i> Please correct the following:</strong><ul>@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div>@endif
                @yield('content')
            </main>
        </div>
    </div>
@else
    ...
    <main id="main-content" class="{{ request()->routeIs('home') ? 'site-main site-main-home' : 'site-main page-shell' }}">
        @if(session('success'))<div class="{{ request()->routeIs('home') ? 'container global-messages' : 'global-messages' }}"><div class="success-box"><i class="fas fa-circle-check"></i><span>{{ session('success') }}</span></div></div>@endif
        @if(session('error'))<div class="{{ request()->routeIs('home') ? 'container global-messages' : 'global-messages' }}"><div class="error-box"><i class="fas fa-circle-exclamation"></i><span>{{ session('error') }}</span></div></div>@endif
        @if($errors->any())<div class="{{ request()->routeIs('home') ? 'container global-messages' : 'global-messages' }}"><div class="error-box"><strong><i class="fas fa-circle-exclamation"></i> Please correct the following:</strong><ul>@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div></div>@endif
        @yield('content')
    </main>
